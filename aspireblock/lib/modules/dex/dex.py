...
def get_pairs(quote_asset='ASP', exclude_pairs=[], max_pairs=12, from_time=None):
...
def get_quotation_pairs(exclude_pairs=[], max_pairs=12, from_time=None, include_currencies=[]):
    ...
    for currency in currencies:
        ...
        for currency_pair in currency_pairs:
            if currency_pair['pair'] == 'ASP/GASP':
    ...
    for currency in quote_assets:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == 'ASP/GASP':  # ASP/GASP always in first
                    top_pairs.insert(0, top_pair)
                else:
                    top_pairs.append(top_pair)
                all_assets += [currency_pair['base_asset'], currency_pair['quote_asset']]

    if ('GASP' in quote_assets) and ('ASP/GASP' not in [p['base_asset'] + '/' + p['quote_asset'] for p in top_pairs]):
        top_pairs.insert(0, {
            'base_asset': 'ASP',
            'quote_asset': 'GASP'
        })
...
def get_assets_supply(assets=[]):
    ...
    if 'ASP' in assets:
        supplies['ASP'] = (util.call_jsonrpc_api("get_supply", {'asset': 'ASP'})['result'], True)
        assets.remove('ASP')
...
def get_markets_list(quote_asset=None, order_by=None):
    ...
    markets = []
    pairs = []
    currencies = ['ASP', 'GASP'] if not quote_asset else [quote_asset]
    ...
    for pair in pairs:
        ...
        if market['base_asset'] == 'ASP' and market['quote_asset'] == 'GASP':
