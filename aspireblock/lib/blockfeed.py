...
def fuzzy_is_caught_up():
    """We don't want to give users 525 errors or login errors if aspireblockd/aspire-server is in the process of
    getting caught up, but we DO if aspireblockd is either clearly out of date with the blockchain, or reinitializing its database"""
...
def process_cp_blockfeed():
    ...
    config.state['my_latest_block'] = {'block_index': 0}  # last block that was successfully processed by aspireblockd
    ...
    if(app_config.count() == 0 or
       ...
       app_config[0]['running_testnet'] != config.TESTNET):
        if app_config.count():
            logger.warn("aspireblockd database version UPDATED (from %i to %i) or testnet setting changed (from %s to %s). REBUILDING FROM SCRATCH ..." % (
                app_config[0]['db_version'], config.DB_VERSION, app_config[0]['running_testnet'],
                config.TESTNET))
        else:
            logger.warn("aspireblockd database app_config collection doesn't exist. BUILDING FROM SCRATCH...")
        ...
        if my_latest_block:
            # remove any data we have for blocks higher than this (would happen if aspireblockd or mongo died
    ...
    # avoid contacting aspire-server (on reparse, to speed up)
    autopilot = False
    autopilot_runner = 0
    iteration = 0
    ...
    # start polling aspire-server for new blocks
    ...
    while True:
        ...
        if not autopilot or autopilot_runner == 0:
            try:
                ...
                logger.warn("Cannot contact aspire-server (via get_running_info): {}".format(e))
                time.sleep(3)
                continue

        # wipe our state data if necessary, if aspire-server has moved on to a new DB version
        wipeState = False
        updatePrefs = False
        ...
        if app_config.get('aspired_db_version_major', None) is None \
           ...
           or app_config.get('aspired_running_testnet', None) is None:
            ...
            logger.warn(
                "aspire-server MAJOR DB version change (we built from %s, aspire-server is at %s). Wiping our state data." % (
                    app_config['aspired_db_version_major'], cp_running_info['version_major']))
            wipeState = True
            updatePrefs = True
        elif cp_running_info['version_minor'] != app_config['aspired_db_version_minor']:
            logger.warn(
                "aspire-server MINOR DB version change (we built from %s.%s, aspire-server is at %s.%s). Wiping our state data." % (
                    app_config['aspired_db_version_major'], app_config['aspired_db_version_minor'],
                    cp_running_info['version_major'], cp_running_info['version_minor']))
            wipeState = True
            updatePrefs = True
        elif cp_running_info.get('running_testnet', False) != app_config['aspired_running_testnet']:
            logger.warn("aspire-server testnet setting change (from %s to %s). Wiping our state data." % (
                app_config['aspired_running_testnet'], cp_running_info['running_testnet']))
            wipeState = True
            updatePrefs = True
        if wipeState:
        ...
        if updatePrefs:
            app_config['aspired_db_version_major'] = cp_running_info['version_major']
            app_config['aspired_db_version_minor'] = cp_running_info['version_minor']
            app_config['aspired_running_testnet'] = cp_running_info['running_testnet']
        ...
        # work up to what block aspired is at
        try:
            if cp_running_info['last_block']:  # should normally exist, unless aspire-server had an error getting it
        ...
        if config.state['my_latest_block']['block_index'] < config.state['cp_latest_block_index']:
            ...
            try:
                ...
                # aspire-server might have gone away...
                my_latest_block = config.mongo_db.processed_blocks.find_one(sort=[("block_index", pymongo.DESCENDING)])
                if my_latest_block:
                    database.rollback(my_latest_block['block_index'])

                # disable autopilot this next iteration to force us to check up against aspire-server
            ...
            # before what aspired is saying if we see this
            logger.error(
                "Very odd: Ahead of aspire-server with block indexes! Pruning back %s blocks to be safe."
            ...
            # ...we may be caught up (to aspired), but aspired may not be (to the blockchain). And if it isn't, we aren't
            config.state['caught_up'] = config.state['cp_caught_up']

            # this logic here will cover a case where we shut down aspireblockd, then start it up again quickly...
            ...
            publish_mempool_tx()
            time.sleep(2)  # aspireblockd itself is at least caught up, wait a bit to query again for the latest block from cpd
