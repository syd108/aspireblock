...
CONFIG_ARGS = [
    ...
    [('--aspire-connect',), {'help': 'the hostname of the aspired JSON-RPC server'}],
    [('--aspire-port',), {'type': int, 'help': 'the port used to communicate with aspired over JSON-RPC'}],
    [('--aspire-user',), {'help': 'the username used to communicate with aspired over JSON-RPC'}],
    [('--aspire-password',), {'help': 'the password used to communicate with aspired over JSON-RPC'}],
    ...
    [('--rpc-port',), {'type': int, 'help': 'port on which to provide the aspireblockd JSON-RPC API'}],
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='aspireblock', description='aspireblock daemon')

    parser.add_argument(
        '-V', '--version', action='version', version="aspireblock %s" % config.VERSION)
    ...
    parser_server = subparsers.add_parser('server', help='Run aspireblock')
    parser_reparse = subparsers.add_parser('reparse', help='Reparse the aspireblock database')
    parser_enmod = subparsers.add_parser('enmod', help='Enable a module')
    parser_enmod.add_argument('module_path', type=str, help='Full Path of module to Enable relative to Aspireblockd directory')
    parser_dismod = subparsers.add_parser('dismod', help='Disable a module')
    parser_dismod.add_argument('module_path', type=str, help='Path of module to Disable relative to Aspireblockd directory')
    ...
    # Handle arguments
    logger.info("aspireblock command specified: {}".format(args.command))
    ...
    logger.info("aspireblock Version %s starting ..." % config.VERSION)
